...
export function FloatingIncidentsWidget() {
  ...
  const totalNotificaciones = activeIncidents.reduce(
    (sum, inc) => sum + inc.notificacionesPendientes,
    0
  );
      ...
      {totalNotificaciones > 0 && isExpanded && (
        <div className="p-2 bg-amber-50 border-t border-amber-200 rounded-b-lg">
          <div className="flex items-center gap-2 text-amber-700 text-xs">
            ...
            <span>{totalNotificaciones} notificación{totalNotificaciones > 1 ? 'es' : ''} pendiente{totalNotificaciones > 1 ? 's' : ''}</span>
          </div>
        </div>
      )}
    </div>
