...
export default function SalidasInventario() {
  ...
  const [formItem, setFormItem] = useState<ItemSalida>({
    sku: '',
    descripcion: '',
    cantidad: 0,
    ubicacion: ''
  ...
  const procesarSalida = async () => {
    ...
    if (!motivo) {
      toast.error("Ingrese el motivo de la salida");
      return;
    }
    ...
    try {
      for (const item of items) {
        await supabase.from('movimientos_inventario').insert({
          ...
          motivo: motivo,
          referencia: referencia || 'Salida manual'
        });
      }

      toast.success("Salida procesada exitosamente");
      setItems([]);
      setMotivo('');
      setReferencia('');
    } catch (error) {
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-3xl font-bold">Salidas Manuales de Inventario</h1>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Input
                placeholder="Número de orden, ticket, etc."
                value={referencia}
            ...
            <div>
              ...
              <Textarea
                placeholder="Describa el motivo de la salida manual"
                value={motivo}
      ...
      {items.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>Items a Dar Salida</CardTitle>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
            ...
            <div className="mt-4">
              <Button onClick={procesarSalida} className="w-full" size="lg">
                Procesar Salida
              </Button>
            </div>
          </CardContent>
