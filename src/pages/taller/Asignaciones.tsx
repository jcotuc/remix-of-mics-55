...
export default function Asignaciones() {
  ...
  const handleAsignar = async (incidenteId: string, grupo: GrupoColaFifo) => {
    ...
    try {
      ...
      const codigoTecnico = profile?.codigo_empleado || 
    ...
    <div className="container mx-auto py-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button 
          variant="outline" 
          size="sm"
          onClick={() => navigate('/taller/configuracion-colas')}
        >
      ...
      <div className="space-y-4">
              ...
              {grupos.map(grupo => {
                ...
                return (
                  <div key={grupo.id} className="flex flex-col">
                    {/* Tarjeta principal */}
                    <div
                      className={`p-4 rounded-xl transition-all duration-200 min-h-[100px] flex flex-col justify-between border shadow-lg ${
                        !hasIncidentes 
                          ? 'bg-muted/30 border-dashed border-border cursor-default opacity-60' 
                          : `${COLORES_CLASES[color] || COLORES_CLASES.orange} cursor-pointer hover:scale-[1.02] active:scale-[0.98]`
                      }`}
                      onClick={() => {
                    ...
                    >
                      <div className="flex items-start justify-between">
                        ...
                        {hasIncidentes && incidentesGrupo.length > 1 && (
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                                variant="ghost"
                                size="icon"
                                className={`h-6 w-6 -mt-1 -mr-1 ${COLORES_TEXTO[color] || 'text-orange-600'} hover:bg-white/20`}
                                onClick={toggleList}
                              >
                      ...
                      {hasIncidentes && (
                        <p className={`text-xs ${COLORES_TEXTO[color] || 'text-orange-600'} opacity-70 text-center font-medium mt-2`}>
                          Toca para asignarme
                        </p>
                      )}

                      <div className={`flex items-center justify-between mt-3 pt-2 ${
                        hasIncidentes ? 'border-t border-white/20' : 'border-t border-border/50'
                      }`}>
                        <Badge 
                          variant={hasIncidentes ? "secondary" : "outline"} 
                          className={`text-xs border-0 ${hasIncidentes ? (COLORES_BADGE[color] || COLORES_BADGE.orange) : ''}`}
                        >
                          {incidentesGrupo.length} en cola
                        </Badge>
                        {hasIncidentes && (
                          <Plus className={`h-5 w-5 ${COLORES_TEXTO[color] || 'text-orange-600'} opacity-80`} />
                        )}
                      </div>
                    </div>

                    {/* Lista expandible de incidentes en espera */}
                    {showList && incidentesGrupo.length > 1 && (
                      <div className="mt-2 p-2 rounded-lg bg-muted/30 border border-border/50 space-y-1.5 max-h-48 overflow-y-auto">
                        ...
                        {incidentesGrupo.slice(1).map(inc => (
                          <div key={inc.id} className="p-2 rounded-md bg-background/80 border border-border/30">
        ...
        {incidentesStockCemaco.length > 0 && (
          <Card className="mt-6">
            <CardHeader>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {incidentesStockCemaco.map(inc => (
                  <div
                    key={inc.id}
                    className="p-3 rounded-lg border bg-amber-500/10 border-amber-400/50"
                  >
                    ...
                    <div className="mt-3 flex gap-2">
                      {inc.status === 'Ingresado' ? (
                        <Button size="sm" variant="outline" onClick={() => handleRevisar(inc)}>
                          Revisar
                        </Button>
                      ) : (
                        <Button size="sm" variant="outline" onClick={() => handleAprobar(inc)}>
                          Aprobar/Rechazar
                        </Button>
                      )}
                    </div>
        ...
        <Card className="bg-muted/30">
          <CardContent className="py-3">
            <p className="text-xs text-muted-foreground text-center">
              Toca una tarjeta con incidentes para asignarte el primer incidente de la cola (FIFO)
            </p>
          </CardContent>
        </Card>
      </div>
